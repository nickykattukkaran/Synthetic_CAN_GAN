...
# Load the dataset
data = pd.read_csv('DoS_attack_dataset.csv')
...
def preprocess_data(df):
    ...
    df['Timestamp_Normalized'] = df['Timestamp'] / max_timestamp if max_timestamp > 0 else 0
    ...
    processed_features = df[['Timestamp_Normalized', 'ID_Normalized', 'RemoteFrame_Normalized', 'DLC_Normalized']].values
    return np.concatenate([processed_features, payload_normalized], axis=1), max_timestamp, max_dlc, max_id, max_rf

processed_data, max_timestamp_global, max_dlc_global, max_id_global, max_rf_global = preprocess_data(data.copy())
...
def generate_synthetic_can_messages(loaded_generator, num_samples, max_timestamp, max_dlc, max_id, max_rf, payload_length=8):
    noise = np.random.normal(0, 1, (num_samples, latent_dim))
    generated_data_normalized = loaded_generator.predict(noise)

    synthetic_messages = []

    for sample in generated_data_normalized:
        timestamp_norm = sample[0]
        id_norm = sample[1]
        rf_norm = sample[2]
        dlc_norm = sample[3]
        payload_norm = sample[4:]

        timestamp = timestamp_norm * max_timestamp
        can_id = hex(int(id_norm * max_id)).lstrip('0x').zfill(4)
        remote_frame = hex(int(rf_norm * max_rf)).lstrip('0x').zfill(3)
...
epochs = 3000
batch_size = 100
generator_model_path = 'generator_dos_model.h5'
discriminator_model_path = 'discriminator_dos_model.h5'
...
synthetic_df = generate_synthetic_can_messages(loaded_generator, num_synthetic_samples, max_timestamp_global, max_dlc_global, max_id_global, max_rf_global, PAYLOAD_LENGTH)

# Saving the generated DataFrame to a CSV file
synthetic_df.to_csv('synthetic_dos_dataframe_generated.csv', index=False)
print(f"\nGenerated synthetic data saved to synthetic_dos_dataframe_generated.csv")
